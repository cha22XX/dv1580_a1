...
void* mem_alloc(size_t size) {
    ...
    //Initialize a pointer to traverse the free list
    Block* current = free_list;

    //Iterate over the free list to find a large enough free block
    while (current) {
    ...
    return NULL;  //If no large enough block is found, return NULL
}
...
int main() {
    ...
    // Initialize the memory pool with a defined size
    //mem_init(POOL_SIZE);
    int* array = (int*)mem_alloc(sizeof(int) * 20);  // Allocate memory for an array of 20 integers
    if (array == NULL) {    // Check if the memory allocation failed
        printf("Memory allocation failed!\n");
        return 1;
    }
    for (int i = 0; i < 20; i++) {   // Fill the array with values ​​from 0 to 19
        array[i] = i;
    }
    for (int i = 0; i < 20; i++) {   // Print the values ​​in the array
        printf("%d ", array[i]);
    }
    printf("\n");
    // mem_free(array);

    int* resized_array = (int*)mem_resize(array, sizeof(int) * 30);  // Resize the array to hold 30 integers
    if (resized_array != NULL) {
        for (int i = 0; i < 30; i++) {     // Print the values ​​in the modified array
            printf("%d ", resized_array[i]);   
        }
        printf("\n");
        mem_free(resized_array);    // Free the memory for the changed array
    }
    
    Node* head;   // Declare a pointer to represent the beginning of the linked list

    list_init(&head,POOL_SIZE);  // Initialize the linked list by setting head to NULL

    // Insert nodes with values ​​2, 4, 5 and 6 into the list
    list_insert(&head, 2);
    list_insert(&head, 4);
    list_insert(&head, 5);
    list_insert(&head, 6);

    list_display(&head);   // Display the contents of the list

    list_insert_after(head,3);   // Insert a new node with value 3 after the first node
    
    list_insert_before(&head,head,1);   // Insert a new node with value 1 before the first node
    
    list_display(&head);   // Display the updated list
    
    list_display_range(&head,head,head);   // Display the contents of the list between the first node and itself(only the first node)

    printf("Node count: %d\n", list_count_nodes(&head));   // Count the nodes in the list and print the result

    list_delete(&head,3);   // Remove the node with value 3 from the list
    
    int search_value=2;
    Node* search_node=list_search(&head,search_value);   // Search for the node with the specified value
    
    // Check if the node with the specified value was found
    if (search_node != NULL) {
        printf("Node %d is exist \n", search_node->data);
    } else {
        printf("Node %d not found.\n", search_value);
    }
    
    list_display(&head);   // Display the list after removing the node

    list_cleanup(&head);   // Clean up and free all nodes in the list to avoid memory leaks

    mem_deinit();     // End memory management and release resources from the memory pool
    return 0;
}
